  ...
  return (
    <Card>
          ...
          <TableBody>
            {products.map((p) => (
              <TableRow key={p.id}>
                ...
                <TableCell style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '12px', color: vars.color.textMuted }}>{p.sku}</TableCell>
                <TableCell style={{ fontVariantNumeric: 'tabular-nums' }}>${p.price.toFixed(2)}</TableCell>
                <TableCell style={{ fontVariantNumeric: 'tabular-nums' }}>{p.stock}</TableCell>
                <TableCell>
      ...
      <CardFooter className={s.tableFooter}>
        <span className={s.kpiDesc}>Showing 1–10 of 156 products</span>
        <Pagination>
          <PaginationContent>
